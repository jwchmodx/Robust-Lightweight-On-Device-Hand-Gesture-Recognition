...
python src/armed/train_gru_armed.py \
  --data-root data/model_ready/armed_24f \
  --epochs 30 \
  --batch-size 256 \
  --hidden-size 128
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train GRU on model-ready Armed-12 gesture dataset."
    )

    parser.add_argument(
        "--data-root",
        default="data/model_ready/armed_24f",
    ...
    run_name = f"gru_armed12_{sequence_length}f"
    if args.use_class_weights:
        run_name += "_weighted"
    ...
    if args.epochs <= 0:
        verify_path = output_dir / "verify_armed12_train_gru.json"
        verify = {
            "mode": "verify_only",
            "status": "ok",
